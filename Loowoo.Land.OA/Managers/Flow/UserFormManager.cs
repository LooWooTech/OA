    ...
    public class UserFormManager:ManagerBase
    {
        ...
        public bool Edit(UserForm userForm)
        {
            using (var db = GetDbContext())
            {
                ...
                if (model == null)
                {
                    model = db.User_Forms.FirstOrDefault(e => e.UserID == userForm.UserID && e.FormID == userForm.FormID && e.InfoID == userForm.InfoID&&e.FlowNodeDataID==userForm.FlowNodeDataID);
                    if (model == null)
                    {
                        return false;
                    }
        ...
        public bool Exist(UserForm userForm)
        {
            using (var db = GetDbContext())
            {
                return db.User_Forms.Any(e => e.UserID == userForm.UserID && e.InfoID == userForm.InfoID && e.FormID == userForm.FormID);
            }
        }
    }
}